...
class AppService {
  ...
  async redirectRoom(req, res) {
    ...
    let { fullUrl, sala } = this.getSalaUrl(req);
  ...
  async room(req, res) {
    ...
    let { fullUrl, sala = 'global' } = this.getSalaUrl(req);
    let { session: { user = null } } = req;

    if(user){

      let { username: nome, _id: user_id, icon } = user;
      const userRender = JSON.stringify({ nome, user_id, icon });
      res.render('room', { bots, user: userRender, sala, url: fullUrl });
  ...
  async upload(req, res) {
    ...
    const { files = null, params: { type = '' } } = req;
    ...
    let { fileUpload: sampleFile } = files;
